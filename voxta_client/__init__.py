...
class VoxtaClient:
    ...
    def on(self, event_name: str, callback: Optional[Callable] = None) -> Any:
        ...
        Can be used as a decorator or a regular method.
        """
    ...
    async def authenticate(self, _connection_token: str):
    ...
    async def register_app(self, label: str = "Voxta Proxy"):
        ...
        payload = {
            ...
            "invocationId": invocation_id,
            "target": "SendMessage",
            "arguments": [{"$type": "registerApp", "clientVersion": "1.2.1", "label": label}],
        }
    ...
    async def _handle_server_message(self, message: dict):
        ...
        if msg_type != 6:
            self.logger.info(
                f"SIGNALR IN: (type {msg_type}, target {target}): {json.dumps(message)}"
            )
        ...
        if msg_type == 1:
            ...
            if arguments:
                ...
                if msg_id and msg_inner_type in [
                    "message",
                    "update",
                    "replyStart",
                    "speechPlaybackStart",
                ...
                if msg_inner_type:
                    ...
                    self.logger.debug(
                        f"Voxta Event (no type, target={target}): {json.dumps(payload)}"
                    )
                ...
                if msg_inner_type == "welcome":
                    ...
                    # should NEVER switch our sessionId to one belonging to another
                    # client (like the Web UI).
                    ...
                    if sessions:
                        ...
                        if not self.chat_id:
                            ...
                            if not found_current:
                                self.logger.warning(
                                    f"Current chat {self.chat_id} no longer in active list!"
                                )
                    ...
                    self.logger.info(
                        f"Chat started, Session ID: {new_session_id}, Chat ID: {new_chat_id}"
                    )
    ...
    async def update_context(
        ...
        if events:
            args["events"] = events
        if set_flags:
            args["setFlags"] = set_flags
        if enable_roles:
            args["enableRoles"] = enable_roles
