...
async def test_connect_flow(mock_websocket):
    ...
    with patch("websockets.connect", return_value=mock_websocket) as mock_connect:
        ...
        with patch.object(client, "_read_loop", new_callable=AsyncMock) as mock_read:
            ...
            auth_msg = next(m for m in mock_websocket.sent_messages if m.get("target") == "SendMessage" and m["arguments"][0]["$type"] == "authenticate")
...
async def test_handle_error_logging():
    ...
    with patch.object(client.logger, 'error') as mock_log:
        # Test generic error
        await client._handle_server_message(wrap_signalr({"$type": "error", "message": "Fatal error"}))
...
async def test_handle_chats_sessions_updated(mock_websocket):
    ...
    sent_subscribe = next(m for m in mock_websocket.sent_messages if m.get("target") == "SendMessage" and m["arguments"][0]["$type"] == "subscribeToChat")
